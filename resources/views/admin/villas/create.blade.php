...
@section('page-description', 'Tambahkan villa baru ke dalam sistem rekomendasi')
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="p-6 bg-primary-600 text-white">
            <h3 class="text-xl font-semibold flex items-center">
                ...
                Form Tambah Villa Baru
            </h3>
            <p class="mt-1 text-primary-100">Lengkapi semua informasi villa dengan detail</p>
        </div>
            ...
            <form action="{{ route('admin.villas.store') }}" method="POST" enctype="multipart/form-data" id="villaForm">
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="type" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('type') border-red-500 @enderror" required>
                                ...
                                <option value="Tradisional" {{ old('type') === 'Tradisional' ? 'selected' : '' }}>Tradisional</option>
                                <option value="Minimalis" {{ old('type') === 'Minimalis' ? 'selected' : '' }}>Minimalis</option>
                                <option value="Resort" {{ old('type') === 'Resort' ? 'selected' : '' }}>Resort</option>
                                <option value="Vila Keluarga" {{ old('type') === 'Vila Keluarga' ? 'selected' : '' }}>Villa Keluarga</option>
                            </select>
                            @error('type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="address" rows="3" 
                                  ...
                                  placeholder="Masukkan alamat lengkap villa..." required>{{ old('address') }}</textarea>
                        @error('address')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="price_max" value="{{ old('price_max') }}" 
                        ...
                        <div>
                            ...
                            <input type="number" name="garage" value="{{ old('garage', 0) }}" 
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="flex items-center">
                            <input type="checkbox" name="swimming_pool" id="swimming_pool" value="1" 
                                   class="rounded border-gray-300 text-primary-600 focus:ring-primary-500" {{ old('swimming_pool') ? 'checked' : '' }}>
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" name="garden" id="garden" value="1" 
                                   class="rounded border-gray-300 text-primary-600 focus:ring-primary-500" {{ old('garden') ? 'checked' : '' }}>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <div>
                            ...
                            <select name="location_score" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('location_score') border-red-500 @enderror" required>
                                ...
                                @for($i = 1; $i <= 5; $i++)
                                    <option value="{{ $i }}" {{ old('location_score') == $i ? 'selected' : '' }}>{{ $i }} - {{ $i == 1 ? 'Sangat Buruk' : ($i == 2 ? 'Buruk' : ($i == 3 ? 'Cukup' : ($i == 4 ? 'Baik' : 'Sangat Baik'))) }}</option>
                                @endfor
                            </select>
                        ...
                        <div>
                            ...
                            <select name="facilities_score" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('facilities_score') border-red-500 @enderror" required>
                                <option value="">Pilih Skor</option>
                                @for($i = 1; $i <= 5; $i++)
                                    <option value="{{ $i }}" {{ old('facilities_score') == $i ? 'selected' : '' }}>{{ $i }} - {{ $i == 1 ? 'Sangat Buruk' : ($i == 2 ? 'Buruk' : ($i == 3 ? 'Cukup' : ($i == 4 ? 'Baik' : 'Sangat Baik'))) }}</option>
                                @endfor
                            </select>
                        ...
                        <div>
                            ...
                            <select name="accessibility_score" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('accessibility_score') border-red-500 @enderror" required>
                                <option value="">Pilih Skor</option>
                                @for($i = 1; $i <= 5; $i++)
                                    <option value="{{ $i }}" {{ old('accessibility_score') == $i ? 'selected' : '' }}>{{ $i }} - {{ $i == 1 ? 'Sangat Buruk' : ($i == 2 ? 'Buruk' : ($i == 3 ? 'Cukup' : ($i == 4 ? 'Baik' : 'Sangat Baik'))) }}</option>
                                @endfor
                            </select>
                        ...
                        <div>
                            ...
                            <select name="security_score" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('security_score') border-red-500 @enderror" required>
                                <option value="">Pilih Skor</option>
                                @for($i = 1; $i <= 5; $i++)
                                    <option value="{{ $i }}" {{ old('security_score') == $i ? 'selected' : '' }}>{{ $i }} - {{ $i == 1 ? 'Sangat Buruk' : ($i == 2 ? 'Buruk' : ($i == 3 ? 'Cukup' : ($i == 4 ? 'Baik' : 'Sangat Baik'))) }}</option>
                                @endfor
                            </select>
                        ...
                        <div>
                            ...
                            <select name="environment_score" class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 @error('environment_score') border-red-500 @enderror" required>
                                <option value="">Pilih Skor</option>
                                @for($i = 1; $i <= 5; $i++)
                                    <option value="{{ $i }}" {{ old('environment_score') == $i ? 'selected' : '' }}>{{ $i }} - {{ $i == 1 ? 'Sangat Buruk' : ($i == 2 ? 'Buruk' : ($i == 3 ? 'Cukup' : ($i == 4 ? 'Baik' : 'Sangat Baik'))) }}</option>
                                @endfor
                            </select>
                        ...
                        <div>
                            <div class="flex items-center h-full">
                                <input type="checkbox" name="is_active" id="is_active" value="1" 
                                       class="rounded border-gray-300 text-primary-600 focus:ring-primary-500" {{ old('is_active', true) ? 'checked' : '' }}>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <textarea name="description" rows="4" 
                                      ...
                                      placeholder="Deskripsikan villa dengan detail untuk menarik tamu...">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-500">Format yang didukung: JPG, JPEG, PNG, GIF. Maksimal 2MB.</p>
                            @error('image')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200">
                    ...
                    <button type="submit" class="px-6 py-3 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition">
                        <i class="fas fa-save mr-2"></i>Simpan Villa
                    </button>
                </div>
            </form>
        </div>
